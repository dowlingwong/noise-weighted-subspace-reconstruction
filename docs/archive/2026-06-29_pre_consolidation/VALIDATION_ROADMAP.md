...
- [x] Add official `H1_DATA`/`L1_DATA` segment metadata, per-window null
  ...
  run.
- [x] Replace the approximate chirp with the documented public GW150914
  numerical-simulation waveform used by GWpy's injection example; archive
  URL, checksum, resampling, placement, and normalization.
- [x] Add a predeclared shared-FIR equivalence sweep over fixed FIR durations
  and edge trims, beginning with a stationary synthetic control.
- [x] Add a predeclared leave-one-out global versus 32/64/96-second local PSD
  comparison with a 64-second primary model, chronological blocks,
  template-projected PSD, and fixed narrow-band power ratios.
