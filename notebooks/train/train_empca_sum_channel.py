...
import sys
# wk4/train -> wk4 -> PCA_dev -> reusable
...
def baseline_correct_per_trace(X_time, pretrigger=2000, method="mean"):
...
def project_coefficients(X_feat, eigvec, weight, reg=1e-12):
    """
    Weighted least-squares projection:
        b = argmin_b ||x - b Phi||_W^2
    where Phi are EMPCA components and W is diagonal (one-sided metric weight).
    """
    X_feat = np.asarray(X_feat)
    eigvec = np.asarray(eigvec)
    w = np.asarray(weight, dtype=np.float64)
    if X_feat.ndim != 2:
        raise ValueError(f"X_feat must be 2D; got {X_feat.shape}")
    if eigvec.ndim != 2:
        raise ValueError(f"eigvec must be 2D; got {eigvec.shape}")
    if w.ndim != 1:
        raise ValueError(f"weight must be 1D; got {w.shape}")

    # Clip negatives to preserve a well-defined metric.
    w = np.clip(w, 0.0, None)
    if not np.any(w > 0):
        raise ValueError("All weights are zero after clipping.")

    phi = eigvec.T  # (n_freq, n_comp)
    if X_feat.shape[1] != phi.shape[0]:
        raise ValueError(
            f"Feature bins mismatch: X_feat has {X_feat.shape[1]} bins, "
            f"but eigvec expects {phi.shape[0]}."
        )
    if w.shape[0] != phi.shape[0]:
        raise ValueError(
            f"Weight length mismatch: weight has {w.shape[0]} bins, "
            f"but eigvec expects {phi.shape[0]}."
        )
    ...
    if reg is not None and reg > 0:
        lhs = lhs + reg * np.eye(lhs.shape[0], dtype=lhs.dtype)

    rhs = (X_feat * w[None, :]) @ phi.conj()
    try:
        coeff = np.linalg.solve(lhs, rhs.T).T
    except np.linalg.LinAlgError:
        coeff = (np.linalg.pinv(lhs) @ rhs.T).T
    return coeff


def weighted_reconstruction_score(
    pca,
    X_time,
    weight,
    *,
    ...
    """
    Validation score used for safe model selection.
    Lower is better.
    """
    if X_time.shape[0] == 0:
        return np.inf
    ...
    eigvec = np.asarray(pca.eigvec)
    coeff = project_coefficients(X_feat, eigvec, weight)
    recon = coeff @ eigvec
    resid = X_feat - recon

    w = np.asarray(weight, dtype=np.float64)
    w = np.clip(w, 0.0, None)
    weighted_power = np.real(np.sum((resid.conj() * resid) * w[None, :], axis=1))
    dof = max(1, X_feat.shape[1] - eigvec.shape[0])
    return float(np.mean(weighted_power / dof))


def select_best_result(results, score_key="val_score"):
    finite = [r for r in results if np.isfinite(r.get(score_key, np.inf))]
    if finite:
        return min(finite, key=lambda d: (d[score_key], d["chi2s"][-1]))
    return min(results, key=lambda d: d["chi2s"][-1])
...
CFG = {
    ...
    "val_frac": 0.25,
    "seed": 0,
    "selection_metric": "val_weighted_recon",
}
    ...
    for noise_name, noise_cfg in NOISE_CONFIGS.items():
        ...
        for r in range(CFG["n_restarts"]):
            ...
            try:
                ...
                val_score = weighted_reconstruction_score(
                    pca,
                    X_val,
                    weights_inv_psd,
                    pretrigger=CFG["pretrigger"],
                ...
                print(
                    f"  Final chi2: {chi2s[-1]:.6g} (iters={len(chi2s)}) "
                    f"val_score={val_score:.6g}"
                )
                results.append(
                    {
                        "pca": pca,
        ...
        if results:
            best = select_best_result(results, score_key="val_score")
            print()
            print(
                f"[{noise_name}/1overPSD] Selected best model by "
                f"{CFG['selection_metric']} (fallback: final chi2)."
            )
            ...
            model_artifact = {
                ...
                "selected_val_score": best.get("val_score", np.nan),
                "selected_train_final_chi2": float(best["chi2s"][-1]),
            }
        ...
        if not snr2_path.exists():
            ...
            if results_b:
                best = select_best_result(results_b, score_key="val_score")
                print()
                print(
                    f"[{noise_name}/SNR2] Selected best model by "
                    f"{CFG['selection_metric']} (fallback: final chi2)."
                )
